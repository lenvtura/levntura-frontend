/**
 * HTTP wrapper around `seedGlobals()` — convenient way to seed the three
 * site-wide globals from a browser/curl without shell access.
 *
 * Usage:
 *   GET /api/admin/seed-globals?secret=<PAYLOAD_PREVIEW_SECRET>
 *
 * For local terminal use prefer:
 *   pnpm seed:globals
 *
 * Both call the exact same `seedGlobals()` function in lib/seeds.
 */
